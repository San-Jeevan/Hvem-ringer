...
public class GulePrivatDataHandler extends DefaultHandler {

	// booleans that check whether it's in a specific tag or not
	private boolean _inFirstName, _inLastName, _inStreet, _inPostCode, _inPostArea, _inStreetNumber;
	...
	public void startElement(String namespaceURI, String localName, String qName, Attributes atts) throws SAXException {
		...
		if (qName.equals("e:first_name")) {
			_inFirstName = true;
		} else if (qName.equals("e:last_name")) {
			_inLastName = true;
		} else if (qName.equals("e:street_name")) {
			_inStreet = true;
		} else if (qName.equals("e:street_number")) {
			_inStreetNumber = true;
	...
	public void characters(char ch[], int start, int length) {
		...
		if (_inFirstName) {
			...
			_data.name = _data.name + (" " + chars);
		} else if (_inStreet) {
			_data.addr = chars;
		} else if (_inStreetNumber) {
			_data.addr= _data.addr +(" " + chars);
		} else if (_inPostArea) {
